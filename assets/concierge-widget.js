...
(function () {
  ...
  // for chainmore.io/demo etc., bullet lists. Intentionally minimal so
  ...
  function mount() {
    ...
    var footer = el('div', { class: 'cm-concierge-footer' }, [
      el('span', { text: 'Discovery call: ' }),
      el('a', {
        href: 'https://chainmore.io/demo',
        target: '_blank',
        rel: 'noopener noreferrer',
        text: 'chainmore.io/demo',
      }),
    ]);
  ...
  function send() {
    ...
    fetch(API_ENDPOINT, {
      ...
      if (res.status === 429) {
        ...
        state.messages.pop();
        showError('Too many messages right now. Please try again in a few minutes, or book a discovery call at chainmore.io/demo.');
        return null;
      }
      if (!res.ok || !res.body) {
        finalizeAssistantMessage(ref);
        if (ref.wrap && ref.wrap.parentNode) ref.wrap.parentNode.removeChild(ref.wrap);
        state.messages.pop();
        showError('Connection issue. Please try again, or book a discovery call at chainmore.io/demo.');
        return null;
      }
        ...
        while ((idx = leftover.indexOf('\n\n')) !== -1) {
          ...
          for (var i = 0; i < lines.length; i++) {
            ...
            if (evt.type === 'delta' && typeof evt.text === 'string') {
              ...
              showError('Something went wrong. Please try again, or book a discovery call at chainmore.io/demo.');
              return;
            }
          }
        }
